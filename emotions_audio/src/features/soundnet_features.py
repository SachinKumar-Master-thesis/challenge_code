...
from emotions_audio.src.features.feature_base import *
...
class SoundNetFeatures(FeaturesBase):
    def __init__(self, pd_params):
        FeaturesBase.__init__(self, pd_params)
        self.v_model_path = pd_params['path']['soundnet']
    ...
    def build_model(self):
        ...
        :return:
        """
        model_weights = np.load(self.v_model_path).item()
        model = Sequential()
    ...
    def build_model_wrapper(self):
        self.m_sound_net = self.build_model()

    def run(self, pma_data):
        lma_data = self.preprocess(audio=pma_data)

        lma_feature =self.m_sound_net.predict(lma_data)
        return lma_feature